...
class AdminApp(ctk.CTk):
    ...
    def create_sidebar(self):
        ...
        ctk.CTkLabel(
            brand_text_frame, text="ADMIN PANEL", font=(FONT, 13, "bold"), text_color=TEXT_PRIMARY
        ).pack(anchor="w")
    ...
    def show_overview_view(self):
        ...
        filter_card.grid(row=1, column=0, sticky="ew", padx=32, pady=(0, 12))

        ctk.CTkLabel(
            filter_card, text="FILTER BY PRIORITY", font=(FONT, 9, "bold"), text_color=TEXT_MUTED
        ...
        self.priority_filter.pack(side="left", padx=(0, 15), pady=9)

        table_card = ctk.CTkFrame(
            self.main_frame, fg_color=BG_INPUT, corner_radius=12,
            border_width=1, border_color=BORDER
        )
        table_card.grid(row=2, column=0, sticky="nsew", padx=32, pady=(0, 25))
        self.main_frame.grid_rowconfigure(2, weight=1)

        columns = ("ID", "Title", "Citizen", "Department", "Status", "Priority")

        self.tree = ttk.Treeview(table_card, columns=columns, show="headings")

        col_widths = {
            "ID": 65, "Title": 280, "Citizen": 165,
            "Department": 165, "Status": 110, "Priority": 100
        }
        ...
        for col in columns:
            ...
            align = "center" if col in ("ID", "Status", "Priority") else "w"

            self.tree.column(col, anchor=align, width=col_widths.get(col, 120))
    ...
    def load_complaints_data(self, *args):
        ...
        try:
            ...
            if selected_priority == "All":
                ...
                query = """
                    SELECT Complaint_ID, Complaint_Title, Citizen_Name,
                           Dept_Name, Status, Priority
                ...
                query = """
                    ...
                    WHERE Priority = :p
                """
                cursor.execute(query, [selected_priority])
            ...
            messagebox.showerror("Query Error", f"Error querying database view:\n{e}")
    ...
    # =====================================================
    # MANAGE COMPLAINTS PAGE  (lookup, then open/close/escalate)
    # =====================================================
    ...
    def show_manage_view(self):
        ...
        lookup_card.grid(row=1, column=0, sticky="ew", padx=32, pady=(0, 15))

        ctk.CTkLabel(
            lookup_card, text="COMPLAINT ID", font=(FONT, 9, "bold"), text_color=TEXT_MUTED
        ).pack(side="left", padx=(18, 10), pady=10)

        self.manage_id_entry = ctk.CTkEntry(
            lookup_card, placeholder_text="e.g. 5", width=140, height=40,
            fg_color=BG_CARD, border_color=BORDER, text_color=TEXT_PRIMARY,
            placeholder_text_color="#5E5B53", corner_radius=8, font=(FONT, FONT_BODY)
        )
        self.manage_id_entry.pack(side="left", padx=5, pady=10)

        lookup_btn = ctk.CTkButton(
            lookup_card, text="Load Complaint   →", height=40, width=170,
            fg_color=GOLD_DARK, hover_color="#8F783F", text_color="#090A0C",
            font=(FONT, FONT_BUTTON, "bold"), corner_radius=8,
            command=self.load_complaint_for_management
        )
        lookup_btn.pack(side="left", padx=(8, 15), pady=10)
        ...
        details_card.grid(row=2, column=0, sticky="ew", padx=32, pady=(0, 15))
        details_card.grid_columnconfigure(0, weight=1)
        details_card.grid_columnconfigure(1, weight=1)

        ctk.CTkLabel(
            details_card, text="COMPLAINT DETAILS", font=(FONT, FONT_SECTION, "bold"), text_color=GOLD
        ).grid(row=0, column=0, columnspan=2, sticky="w", padx=25, pady=(17, 8))

        self.manage_info_label = ctk.CTkLabel(
            details_card,
            text="Enter a Complaint ID above and select 'Load Complaint' to begin.",
            font=(FONT, 11), text_color=TEXT_SECONDARY, justify="left", anchor="w"
        )
        self.manage_info_label.grid(
            row=1, column=0, columnspan=2, sticky="w", padx=25, pady=(0, 15)
        )

        # Status control
        status_frame = ctk.CTkFrame(details_card, fg_color="transparent")
        status_frame.grid(row=2, column=0, sticky="ew", padx=25, pady=6)

        ctk.CTkLabel(
            status_frame, text="Status", font=(FONT, FONT_LABEL, "bold"), text_color=TEXT_SECONDARY
        ).pack(anchor="w", pady=(0, 4))

        self.manage_status_combo = ctk.CTkComboBox(
            status_frame, values=["Open", "In Progress", "Closed"], height=40,
        ...
        self.manage_status_combo.pack(fill="x")

        # Priority control
        priority_frame = ctk.CTkFrame(details_card, fg_color="transparent")
        priority_frame.grid(row=2, column=1, sticky="ew", padx=25, pady=6)

        ctk.CTkLabel(
            priority_frame, text="Priority", font=(FONT, FONT_LABEL, "bold"), text_color=TEXT_SECONDARY
        ).pack(anchor="w", pady=(0, 4))
        ...
        self.manage_priority_combo.pack(fill="x")

        divider = ctk.CTkFrame(details_card, height=1, fg_color=BORDER)
        divider.grid(row=3, column=0, columnspan=2, sticky="ew", padx=25, pady=(12, 12))

        # -------------------------------------------------
        # ACTION BUTTONS
        # -------------------------------------------------

        actions_frame = ctk.CTkFrame(details_card, fg_color="transparent")
        actions_frame.grid(row=4, column=0, columnspan=2, sticky="ew", padx=25, pady=(0, 18))

        ctk.CTkButton(
            actions_frame, text="Open Complaint", height=40, width=155,
            fg_color="transparent", border_width=1, border_color=SUCCESS,
            text_color=SUCCESS, hover_color="#1C1F1A",
            font=(FONT, FONT_BUTTON, "bold"), corner_radius=9,
            command=lambda: self.set_complaint_status("Open")
        ).pack(side="left", padx=(0, 8))

        ctk.CTkButton(
            actions_frame, text="Close Complaint", height=40, width=155,
            fg_color="transparent", border_width=1, border_color=DANGER,
            text_color=DANGER, hover_color="#1F1A1A",
            font=(FONT, FONT_BUTTON, "bold"), corner_radius=9,
            command=lambda: self.set_complaint_status("Closed")
        ).pack(side="left", padx=8)

        ctk.CTkButton(
            actions_frame, text="Escalate Priority   ↑", height=40, width=175,
            fg_color="transparent", border_width=1, border_color=GOLD,
            text_color=GOLD, hover_color="#211F1A",
            font=(FONT, FONT_BUTTON, "bold"), corner_radius=9,
            command=self.escalate_complaint
        ).pack(side="left", padx=8)

        ctk.CTkButton(
            actions_frame, text="Save Changes   →", height=40, width=170,
            fg_color=GOLD_DARK, hover_color="#8F783F", text_color="#090A0C",
            font=(FONT, FONT_BUTTON, "bold"), corner_radius=9,
    ...
    def _get_manage_complaint_id(self):
        ...
        if not raw_id.isdigit():
            ...
            messagebox.showwarning("Input Error", "Please enter a valid numeric Complaint ID.")
            return None
    ...
    def load_complaint_for_management(self):
        ...
        try:
            ...
            cursor.execute(
                """
                SELECT c.COMPLAINT_TITLE, ci.NAME, d.DEPT_NAME,
            ...
            if not row:
                ...
                messagebox.showinfo(
                    "Not Found", f"No complaint found with ID {complaint_id}."
                )
                self.manage_loaded_id = None
                return

            title, citizen_name, dept_name, status, priority, comp_date = row
            ...
            self.manage_status_combo.set(status if status else "Open")
            self.manage_priority_combo.set(priority if priority else "Medium")

            self.manage_loaded_id = complaint_id

        except Exception as e:

            messagebox.showerror("Query Error", f"Error loading complaint:\n{e}")
    ...
    def _update_complaint(self, complaint_id, status, priority):
        ...
        try:
            ...
            cursor.execute(
                ...
                """,
                {"status": status, "priority": priority, "id": complaint_id}
            )
    ...
    def set_complaint_status(self, new_status):
        ...
        if self.manage_loaded_id is None:
            ...
            messagebox.showwarning(
                "No Complaint Loaded", "Load a complaint first before changing its status."
            )
            return

        priority = self.manage_priority_combo.get()

        if self._update_complaint(self.manage_loaded_id, new_status, priority):

            self.manage_status_combo.set(new_status)

            messagebox.showinfo(
                "Status Updated",
                f"Complaint #{self.manage_loaded_id} marked as '{new_status}'."
            )
    ...
    def escalate_complaint(self):
        ...
        if self.manage_loaded_id is None:
            ...
            messagebox.showwarning(
                "No Complaint Loaded", "Load a complaint first before escalating it."
            )
            return
        ...
        if current_index == len(priority_order) - 1:
            ...
            messagebox.showinfo(
                ...
                f"Complaint #{self.manage_loaded_id} is already marked High priority."
            )
            return
        ...
        if self._update_complaint(self.manage_loaded_id, status, new_priority):

            self.manage_priority_combo.set(new_priority)

            messagebox.showinfo(
                "Complaint Escalated",
                f"Complaint #{self.manage_loaded_id} escalated to '{new_priority}' priority."
            )
    ...
    def save_complaint_changes(self):
        ...
        if self.manage_loaded_id is None:
            ...
            messagebox.showwarning(
                "No Complaint Loaded", "Load a complaint first before saving changes."
            )
            return
        ...
        if self._update_complaint(self.manage_loaded_id, status, priority):
    ...
    def show_report_view(self):
        ...
        report_card.grid(row=1, column=0, sticky="nsew", padx=32, pady=(0, 25))
        report_card.grid_columnconfigure(0, weight=1)
        report_card.grid_rowconfigure(2, weight=1)

        self.main_frame.grid_rowconfigure(1, weight=1)

        controls = ctk.CTkFrame(
            report_card, fg_color=BG_CARD, corner_radius=11,
            border_width=1, border_color=BORDER
        )
        controls.grid(row=0, column=0, sticky="ew", padx=20, pady=18)

        ctk.CTkLabel(
            controls, text="REPORT PARAMETERS", font=(FONT, 9, "bold"), text_color=GOLD
        ).pack(side="left", padx=(18, 12), pady=12)

        self.dept_id_entry = ctk.CTkEntry(
            controls, placeholder_text="Department ID", width=155, height=40,
            fg_color=BG_INPUT, border_color=BORDER, text_color=TEXT_PRIMARY,
            placeholder_text_color="#5E5B53", corner_radius=8, font=(FONT, FONT_BODY)
        )
        self.dept_id_entry.pack(side="left", padx=5, pady=10)

        run_btn = ctk.CTkButton(
            controls, text="Generate Report   →", height=40, width=180,
            fg_color=GOLD_DARK, hover_color="#8F783F", text_color="#090A0C",
            font=(FONT, FONT_BUTTON, "bold"), corner_radius=8,
            command=self.execute_dept_report
        )
        run_btn.pack(side="left", padx=(8, 15), pady=10)

        output_header = ctk.CTkFrame(report_card, fg_color="transparent")
        output_header.grid(row=1, column=0, sticky="ew", padx=25, pady=(0, 7))

        ctk.CTkLabel(
            output_header, text="GENERATED REPORT", font=(FONT, 9, "bold"), text_color=GOLD
        ).pack(side="left")

        ctk.CTkLabel(
            output_header, text="PL/SQL OUTPUT", font=(FONT, 9, "bold"), text_color=TEXT_MUTED
        ).pack(side="right")

        self.output_box = ctk.CTkTextbox(
            report_card, fg_color=BG_MAIN, font=(FONT, 11), text_color=TEXT_SECONDARY,
            corner_radius=10, border_color=BORDER, border_width=1
        )
        self.output_box.grid(row=2, column=0, sticky="nsew", padx=25, pady=(0, 25))
    ...
    def execute_dept_report(self):
        ...
        if not dept_id.isdigit():
            ...
            messagebox.showwarning("Input Error", "Please enter a valid numeric Department ID.")
            return
        ...
        try:
            ...
            cursor.callproc("prc_department_report", [int(dept_id)])
            ...
            while True:
                ...
                output_text += line.getvalue() + "\n"

            self.output_box.delete("1.0", tk.END)

            self.output_box.insert(
                tk.END,
                output_text if output_text.strip() else "No output returned."
            )

        except Exception as e:

            messagebox.showerror("Execution Error", f"Error executing PL/SQL procedure:\n{e}")
